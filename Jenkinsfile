  ...
  stages {
    stage('Build') {
		...
		script {    sh(script:"""printenv""")
		  	    ...
		  	    docker.withRegistry('https://europe-west1-docker.pkg.dev/phewas-development/fg-phewas-registry', 'jenkins-sa') { c.push("ci-${env.GIT_COMMIT}") }
			    docker.withRegistry('https://europe-west1-docker.pkg.dev/phewas-development/fg-phewas-registry', 'jenkins-sa') { c.push("ci-latest") }
		}
