...
class TestTabixResultDao(unittest.TestCase):
    ...
    def test_get_single_variant_results(self):
        ...
        # print(self.data[0])
        # print(self.mocked_pheno_list_data)
        tabix_results = TabixResultFiltDao(self.mocked_pheno_list_data, mock_data_file_path, self.columns)
        split_query=re.split('-|:|/|_', mocked_variant)
        variant = Variant(split_query[0], split_query[1], split_query[2], split_query[3])
        self.assertTrue(len(tabix_results.get_single_variant_results(variant)) > 0)
    
    # def test_get_variants_results(self):
    #     # Check get_variants_results
    #     print(self.data[0])
    #     tabix_results = TabixResultDao(phenos, mock_data_file_path, mocked_columns)
    #     split_query=re.split('-|:|/|_', mocked_variant)
    #     variant = Variant(split_query[0], split_query[1], split_query[2], split_query[3])
    #     self.assertIsNotNone(tabix_results.get_variants_results(variant))
    
    # def test_get_top_per_pheno_variant_results_range(self):
    #     # Check get_top_per_pheno_variant_results_range
    #     print(self.data[0])
    #     tabix_results = TabixResultDao(phenos, mock_data_file_path, mocked_columns)
    #     split_query=re.split('-|:|/|_', mocked_variant)
    #     self.assertEqual(tabix_results.get_top_per_pheno_variant_results_range('chr'+ split_query[0], int(split_query[1]), int(split_query[1])), [])
    ...
    #     tabix_results = TabixResultDao(phenos, mock_data_file_path, mocked_columns)
    #     split_query=re.split('-|:|/|_', mocked_variant)
    #     self.assertEqual(tabix_results.get_variant_results_range('chr'+ split_query[0], int(split_query[1]), int(split_query[1])), [])
